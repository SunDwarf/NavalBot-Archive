...
async def game(client: discord.Client, message: discord.Message):
    ...
    if message.author.permissions_in(message.channel).manage_roles and len(game) < 64:
        ...
        # save it in the DB
        bot.cursor.execute("""INSERT OR REPLACE INTO configuration (id, name, value)
...
async def lock(client: discord.Client, message: discord.Message):
    ...
    bot.cursor.execute("SELECT locked, locker FROM factoids WHERE name = ?", (fac,))
    ...
    bot.cursor.execute("""UPDATE factoids SET locked = 1, locker = ? WHERE name = ?""",
